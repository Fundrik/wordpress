...
final class RestPreInsertCampaignFilterBridge implements HookToEventBridgeInterface {
	...
	/**
	 * Registers the 'rest_pre_insert_(post_type)' WordPress filter and and bridge it to the internal events.
	 *
	...
	public function handle( mixed $prepared_post, mixed $request ): mixed {
		...
		try {
			...
			$this->logger->warning(
				$e->getMessage(),
				LoggerFormatter::hook_bridge_context( hook: $this->get_hook_name(), bridge: self::class ),
			);
